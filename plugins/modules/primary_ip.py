...
EXAMPLES = """
- name: Create a basic IPv4 Primary IP assigned to a Datacenter
  hetzner.hcloud.primary_ip:
    name: my-primary-ip
    datacenter: fsn1-dc14
    type: ipv4
    state: present
- name: Create a basic IPv6 Primary IP assigned to a Datacenter
  hetzner.hcloud.primary_ip:
    name: my-primary-ip
    datacenter: fsn1-dc14
    type: ipv6
...
- name: Ensure the server is stopped
  hetzner.hcloud.server:
    name: my-server
    state: stopped
- name: Create a basic IPv4 Primary IP attached to a Server
  hetzner.hcloud.primary_ip:
    name: my-primary-ip
    server: my-server
    type: ipv4
    state: present
- name: Create a basic IPv6 Primary IP attached to a Server
  hetzner.hcloud.primary_ip:
    name: my-primary-ip
    server: my-server
    type: ipv6
...
class AnsibleHCloudPrimaryIP(AnsibleHCloud):
    ...
    def _prepare_result(self):
        assignee_id = (
            None if self.hcloud_primary_ip.assignee_id is None else to_native(self.hcloud_primary_ip.assignee_id)
        )
        return {
    ...
    def _create_primary_ip(self):
        ...
        self.module._check_required_one_of(["server", "datacenter"], self.module.params)
        try:
            params = {
